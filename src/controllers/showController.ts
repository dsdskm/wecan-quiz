...
export const getShow = async (req: Request, res: Response) => {
    try {
        ...
        Logger.error(`Error fetching show ${req.params.id}:`, error);
        res.status(500).json({ message: error.message });
    }
};
...
export const updateShow = async (req: Request, res: Response) => {
    try {
        ...
        Logger.error(`Error updating show ${req.params.id}:`, error);
        res.status(500).json({ message: error.message });
    }
};
...
export const deleteShow = async (req: Request, res: Response) => {
    try {
        ...
        Logger.error(`Error deleting show ${req.params.id}:`, error);
        res.status(500).json({ message: error.message });
    }
};
...
export const uploadShowBackgroundImage = async (req: Request, res: Response) => {
    try {
        ...
        Logger.error(`Error uploading background image for show ${req.params.showId}:`, error);
        res.status(500).json({ message: error.message });
    }
};
